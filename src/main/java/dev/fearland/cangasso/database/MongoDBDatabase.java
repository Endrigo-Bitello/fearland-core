...
import dev.fearland.cangasso.database.conversor.MongoDBConversor;
  ...
  public Map<String, Map<String, DataContainer>> load(String name) throws ProfileLoadException {
    ...
    for (DataTable table : DataTable.listTables()) {
      ...
      if (table.getInfo().name().contains("SkyWars") || table.getInfo().name().contains("BedWars") || table.getInfo().name().contains("TheBridge")) {
        ...
        String beds = table.getInfo().name().contains("BedWars") ? "beds" : "";
        for (String stats : new String[] {"kills", "deaths", "points", assists, beds, "wins", "games"}) {
